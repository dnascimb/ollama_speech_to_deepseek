...
parser.add_argument("--ollama-model", type=str, help="Optional: forward transcription to an Ollama model (e.g., deepseek, llama3).")
    ...
    try:
        ...
        for line in resp.iter_lines():
            if line:
                ...
                if "response" in data:
                    print(f"[Ollama]: {data['response']}", end="", flush=True)
...
if args.file:
    ...
    if args.ollama_model and text:
        send_to_ollama(text, args.ollama_model)
...
# WAV file → Whisper → DeepSeek (via Ollama):python stt.py --file example.wav --ollama-model deepseek
